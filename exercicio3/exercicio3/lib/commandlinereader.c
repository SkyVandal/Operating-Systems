...
*/
int readLineArgumentsFile(char **argVector, int vectorSize, char *buffer, int bufferSize, FILE* to_read)
{
  int numTokens = 0;
  char *s = " \r\n\t";
  ...
  if (fgets(buffer, bufferSize, to_read) == NULL)
     return -1;
...
void readLineArguments(char **argVector, int vectorSize, char *buffer, int bufferSize){
    readLineArgumentsFile(argVector, vectorSize, buffer, bufferSize, stdin);
}
